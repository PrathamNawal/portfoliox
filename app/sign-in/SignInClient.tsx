...
export function SignInClient() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // Use the Google credential token (not the Firebase token) — tokeninfo verifies Google-issued tokens
      const credential = GoogleAuthProvider.credentialFromResult(result)
      if (!credential?.idToken) throw new Error('No credential returned')
      const idToken = credential.idToken

      // Send token to server → sets httpOnly session cookie
      ...
      // Hard navigate so server sees the new cookie immediately
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'var(--px-bg)', padding: 'clamp(24px, 5vw, 48px) clamp(20px, 6vw, 48px)' }}>
        <div style={{ width: '100%', maxWidth: 360 }}>
          ...
          <button
            ...
            disabled={loading}
            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '12px 16px', fontSize: 14, fontWeight: 600, background: 'var(--px-surface)', border: '1.5px solid var(--px-border)', borderRadius: 'var(--px-r)', cursor: loading ? 'wait' : 'pointer', color: 'var(--px-text)', fontFamily: 'var(--px-font)', transition: 'background 0.15s', opacity: loading ? 0.7 : 1 }}
            onMouseEnter={e => { if (!loading) (e.currentTarget as HTMLElement).style.background = 'var(--px-surface-2)' }}
          ...
          {error && (
            <p style={{ marginTop: 16, fontSize: 13, color: 'var(--px-accent)', textAlign: 'center' }}>{error}</p>
          )}

          <p style={{ fontSize: 12, color: 'var(--px-text-3)', marginTop: 24, textAlign: 'center', lineHeight: 1.5 }}>
