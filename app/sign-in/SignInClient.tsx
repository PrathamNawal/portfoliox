...
export function SignInClient() {
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      const result = await (app as any).sendMagicLinkEmail(email.trim())
      if (result?.status === 'error') {
        setError(result.error?.message || 'Failed to send email. Try again.')
        setSending(false)
      } else {
        setSent(true)
      }
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to send email. Try again.')
      setSending(false)
    }
  }
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  background: 'var(--px-surface)', border: '1.5px solid var(--px-border)',
                  borderRadius: 'var(--px-r)', cursor: googleLoading ? 'not-allowed' : 'pointer',
                  color: 'var(--px-text)', fontFamily: 'var(--px-font)',
              ...
              <form onSubmit={handleMagicLink} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--px-text-2)', marginBottom: 6, letterSpacing: '0.01em' }}>Email address</label>
                  <input
                    type="email"
                    value={email}
                    ...
                    style={{
                      ...
                      border: `1.5px solid ${error ? 'var(--px-accent)' : 'var(--px-border)'}`,
                      borderRadius: 'var(--px-r)', background: 'var(--px-surface)',
                      color: 'var(--px-text)', fontFamily: 'var(--px-font)', outline: 'none',
                      transition: 'border-color 0.15s', boxSizing: 'border-box',
                    }}
                    onFocus={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--px-accent)' }}
                    onBlur={e => { (e.currentTarget as HTMLElement).style.borderColor = error ? 'var(--px-accent)' : 'var(--px-border)' }}
                  />
                ...
                <button
                  ...
                  style={{
                    ...
                    background: sending || !email.trim() ? 'var(--px-surface-2)' : 'var(--px-accent)',
                    color: sending || !email.trim() ? 'var(--px-text-3)' : '#fff',
                    border: 'none', borderRadius: 'var(--px-r)',
                    cursor: sending || !email.trim() ? 'not-allowed' : 'pointer',
                    fontFamily: 'var(--px-font)', transition: 'background 0.15s',
                  }}
                >
