...
export function CanvasLayout({ profile, caseStudies, testimonials, experience, tools }: Props) {
  ...
  return (
    // ── use var(--px-bg) so theme changes propagate ───────────────
    <div style={{ minHeight: '100vh', background: 'var(--px-bg)' }}>
      ...
      <header className="px-canvas-header" style={{ background: 'var(--px-surface)', borderBottom: '1px solid var(--px-border)', padding: '0 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, minWidth: 0 }}>
          {profile.avatar_url
            ...
            : <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--px-accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><span style={{ fontSize: 11, fontWeight: 700, color: 'var(--px-accent-fg)' }}>{profile.name[0]}</span></div>
          }
      ...
      <section className="px-canvas-hero" style={{
        ...
        {profile.avatar_url
          ...
          : <div className="px-hero-avatar" style={{ width: 100, height: 100, borderRadius: 28, background: 'var(--px-accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
              <span style={{ fontSize: 38, fontWeight: 700, color: 'var(--px-accent-fg)' }}>{profile.name[0]}</span>
            </div>
        }
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          {(social as any).behance  && <OutlineBtn href={`https://behance.net/${(social as any).behance}`}>Behance</OutlineBtn>}
          ...
          <a href="mailto:?subject=Hello" style={{ display: 'inline-flex', alignItems: 'center', height: 40, padding: '0 20px', fontSize: 13, fontWeight: 600, borderRadius: 'var(--px-r)', background: 'var(--px-accent)', color: 'var(--px-accent-fg)', textDecoration: 'none' }}>
            Get in touch
          </a>
        </div>
      </section>
      ...
      <main className="px-canvas-main" style={{ maxWidth: 1100, margin: '0 auto', padding: '56px 40px 80px' }}>
            ...
            <div style={{ overflow: 'hidden', marginTop: 20, maskImage: 'linear-gradient(to right, transparent, black 8%, black 92%, transparent)', WebkitMaskImage: 'linear-gradient(to right, transparent, black 8%, black 92%, transparent)' }}>
              <div style={{ display: 'flex', gap: 8, width: 'max-content', animation: 'px-marquee 22s linear infinite' }}>
                {[...profile.skills, ...profile.skills].map((s, i) => (
                  <span key={i} style={{ padding: '7px 16px', fontSize: 13, fontWeight: 500, borderRadius: 'var(--px-r-pill)', background: 'var(--px-surface)', border: '1px solid var(--px-border)', color: 'var(--px-text-2)', whiteSpace: 'nowrap', flexShrink: 0 }}>{s}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        {caseStudies.length > 0 && (
          ...
          <div style={{ marginBottom: 56 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 24 }}>
              <h2 style={{ fontSize: 28, fontWeight: 800, letterSpacing: 'var(--px-tracking-tight)', color: 'var(--px-text)', lineHeight: 1 }}>Case Studies</h2>
              ...
              <div style={{ display: 'flex', gap: 20, width: 'max-content', animation: 'px-marquee 48s linear infinite', padding: '4px 40px 4px' }}>
                {[...testimonials, ...testimonials].map((t, i) => (
                  <div key={i} style={{
                    ...
                    borderRadius: 'var(--px-r-lg)',
                    padding: '32px 32px 28px',
                    boxShadow: 'var(--px-shadow)',
                    display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
                  }}>
                    <p style={{ fontSize: 16, fontWeight: 400, color: 'var(--px-text)', lineHeight: 1.7, marginBottom: 28 }}>
                      &ldquo;{t.quote}&rdquo;
                    </p>
        ...
        <div style={{ borderTop: '1px solid var(--px-border)', paddingTop: 56, marginTop: 8 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {profile.resume_url && (
              <a href={profile.resume_url} target="_blank" rel="noreferrer" className="px-work-link" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', background: 'var(--px-surface)', border: '1px solid var(--px-border)', borderRadius: 'var(--px-r-lg)', textDecoration: 'none', transition: `border-color var(--px-dur-fast)` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div className="px-cta-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              {(social as Record<string,string>).website && (
                <a href={(social as Record<string,string>).website} target="_blank" rel="noreferrer" className="px-work-link" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '20px 24px', background: 'var(--px-surface)', border: '1px solid var(--px-border)', borderRadius: 'var(--px-r-lg)', textDecoration: 'none', transition: `border-color var(--px-dur-fast)` }}>
                  <span style={{ fontSize: 22 }}>🔗</span>
                  <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--px-text)' }}>Website</span>
                </a>
              )}
              <a href={`mailto:${(profile as any).email || ''}`} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '20px 24px', background: 'var(--px-accent)', border: '1px solid var(--px-accent)', borderRadius: 'var(--px-r-lg)', textDecoration: 'none' }}>
                <span style={{ fontSize: 22 }}>✉️</span>
                <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--px-accent-fg)' }}>Get in touch</span>
              </a>
            </div>
          </div>
        </div>
...
function CanvasCaseCard({ cs, index, slug }: { cs: CaseStudy; index: number; slug: string }) {
  ...
  return (
    <Link href={`/${slug}/case/${cs.id}`} className="px-canvas-case-card" style={{ display: 'block', background: 'var(--px-surface)', borderRadius: 'var(--px-r-lg)', border: '1px solid var(--px-border)', overflow: 'hidden', textDecoration: 'none', transition: `box-shadow var(--px-dur-base), transform var(--px-dur-base)` }}>
      <div style={{ height: 220, position: 'relative', overflow: 'hidden' }}>
        ...
        {cs.nda_enabled && (
          <div style={{ position: 'absolute', inset: 0, backdropFilter: 'blur(10px)', background: 'rgba(0,0,0,0.65)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            <div style={{ width: 40, height: 40, borderRadius: 'var(--px-r-pill)', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1.5px solid rgba(255,255,255,0.2)' }}>
      ...
      <div style={{ padding: '16px 20px 20px' }}>
        {disciplineLabel && cs.cover_image_url && !cs.nda_enabled && (
          <span style={{
            ...
            textTransform: 'uppercase', padding: '3px 9px', borderRadius: 'var(--px-r-pill)',
        ...
        {summary && !cs.nda_enabled && (
          <p style={{ fontSize: 13, color: 'var(--px-text-3)', lineHeight: 1.55, margin: 0, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {summary}
          </p>
        )}
        {cs.nda_enabled && (
        ...
        {!cs.nda_enabled && (
          <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--px-accent)' }}>View project</span>
            <span style={{ fontSize: 12, color: 'var(--px-accent)' }}>→</span>
          </div>
        )}
      </div>
    </Link>
...
function PublishedToolIcon({ name }: { name: string }) {
  ...
  return (
    <div className="px-tool-icon" style={{
      ...
      // ── uses motion tokens ───────────────────────────────────────
      transition: `transform var(--px-dur-base) var(--px-ease-hover), box-shadow var(--px-dur-base) var(--px-ease-hover)`,
      flexShrink: 0,
      overflow: 'hidden',
    }}>
      {slug
      ...
      <div className="px-tool-name" style={{
        ...
        // ── uses motion tokens ───────────────────────────────────────
        transition: `transform var(--px-dur-fast) var(--px-ease-hover)`,
        zIndex: 2,
      }}>
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <h2 style={{ fontSize: 26, fontWeight: 800, letterSpacing: 'var(--px-tracking-tight)', color: 'var(--px-text)', lineHeight: 1, marginBottom: 0 }}>
      {children}
    </h2>
  )
}
...
function SocialLink({ href, label }: { href: string; label: string }) {
  return (
    <a href={href} target="_blank" rel="noreferrer" className="px-social-link" style={{ fontSize: 12, color: 'var(--px-text-3)', textDecoration: 'none', fontWeight: 500, padding: '4px 8px', borderRadius: 'var(--px-r-sm)', transition: `background var(--px-dur-micro)` }}>
      {label}
    </a>
  )
}
...
function CTALink({ children }: { children: React.ReactNode }) {
  return (
    <a href="mailto:?subject=Hello" style={{ display: 'inline-flex', alignItems: 'center', height: 32, padding: '0 14px', fontSize: 13, fontWeight: 600, borderRadius: 'var(--px-r)', background: 'var(--px-accent)', color: 'var(--px-accent-fg)', textDecoration: 'none' }}>{children}</a>
  )
}
